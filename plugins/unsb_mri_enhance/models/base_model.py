...
class BaseModel(ABC):
    ...
    def setup(self, opt):
        ...
        if self.isTrain and opt.continue_train:
            if opt.pretrained_name is not None:
                resume_dir = os.path.join(opt.checkpoints_dir, opt.pretrained_name) if not os.path.isabs(opt.pretrained_name) else opt.pretrained_name
            else:
                resume_dir = self.save_dir
            print('[resume] continue_train=1')
            print('[resume] load epoch tag: %s' % opt.epoch)
            print('[resume] source dir: %s' % resume_dir)
    ...
    def _safe_torch_load(path, map_location):
        """Load torch checkpoint with weights_only=True when supported."""
        try:
    ...
    def load_networks(self, epoch):
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                # Some checkpoints contain the same key names but different tensor shapes
                # (e.g., anti-aliased filter buffers). Remove only the shape-matched
                # entries to avoid runtime errors during load_state_dict.
