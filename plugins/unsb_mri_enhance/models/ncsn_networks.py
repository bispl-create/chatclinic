...
class NLayerDiscriminator_ncsn(nn.Module):
    ...
    def forward(self, input, t_emb, input2=None, return_feats: bool = False):
        """Standard forward."""
...
class ResnetGenerator_ncsn(nn.Module):
    ...
    def forward(self, x, time_cond, z, layers=[], encode_only=False, ctrl=None):
        z_embed = self.z_transform(z)
        temb = get_timestep_embedding(time_cond, self.ngf)
        time_embed = self.time_embed(temb)

        # PatchNCE feature extraction path (ctrl ignored)
        if len(layers) > 0:
            feat = x
            feats = []
        ...
        # Standard generation path with optional ControlNet injections
        out = x
        down_count = 0
        down_pending = False
        for layer in self.model:
            ...
            if isinstance(layer, Downsample):
                down_count += 1
                if down_count == 1 and 'down1' in ctrl:
                    out = out + ctrl['down1']
                elif down_count == 2 and 'down2' in ctrl:
                    out = out + ctrl['down2']
            elif isinstance(layer, nn.Conv2d) and layer.stride == (2, 2):
                down_pending = True
            elif down_pending and isinstance(layer, nn.ReLU):
                down_count += 1
                down_pending = False
                if down_count == 1 and 'down1' in ctrl:
                    out = out + ctrl['down1']
        ...
        for i, layer in enumerate(self.model_res):
        ...
        for layer in self.model_upsample:
            ...
            if isinstance(layer, Upsample):
                up_pending = True
            elif isinstance(layer, nn.ConvTranspose2d) and layer.stride == (2, 2):
                up_pending = True
            elif up_pending and isinstance(layer, nn.ReLU):
                up_count += 1
                up_pending = False
                if up_count == 1 and 'up1' in ctrl:
                    out = out + ctrl['up1']
                elif up_count == 2 and 'up2' in ctrl:
                    out = out + ctrl['up2']
