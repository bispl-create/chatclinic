...
from .controlnet_ncsn import ControlNetNCSN
...
def define_C(cond_nc, ngf, norm='instance', init_type='normal', init_gain=0.02, gpu_ids=[], opt=None):
    """Create a ControlNet (conditioning) network."""
    norm_layer = get_norm_layer(norm_type=norm)
    net = ControlNetNCSN(cond_nc=cond_nc, ngf=ngf, norm_layer=norm_layer, opt=opt)
    return init_net(net, init_type, init_gain, gpu_ids)
